...
#define BLINK_GPIO 2
int led = 0;
dataTemperature_t dataTemp;
dataHumidity_t dataHum;
...
static bool mqtt_mosquitto_client_configure(void);
...
void mqtt_app_main_task(void *parm)
{
    ...
    mqtt_mosquitto_client_configure();
    esp_mqtt_client_handle_t mqtt_client_handle = esp_mqtt_client_init(&mqtt_cfg);
    ...
    while (1)
    {
        ...
        if (mqtt_client_configure())
        {
            ...
            esp_mqtt_set_config(mqtt_client_handle, &mqtt_cfg);
...
static bool mqtt_mosquitto_client_configure(void)
{
    ...
    // mqtt_cfg.client_cert_pem = (const char *)client_cert_pem_start;
    // mqtt_cfg.client_key_pem = (const char *)client_key_pem_start;
    //  mqtt_cfg.cert_pem = (const char *)server_cert_pem_start;
    mqtt_cfg.username = MOSQUITO_USER_NAME;
    mqtt_cfg.password = MOSQUITO_USER_PASSWORD;
...
void publicar_temperatura_task(void *parm)
{
    ...
    char buffer_luz1_txt[4];
    char buffer_luz2_txt[4];
    ...
    while (1)
    {
        if (mqtt_client_connected == true)
        {
            ...
            getDataTemperature(&dataTemp);
            getDataHumidity(&dataHum);
            led = ~led;
            ...
            snprintf(buffer_temp_txt, sizeof(buffer_temp_txt), "%.2f", dataTemp.ActualTemperature);
            snprintf(buffer_hum_txt, sizeof(buffer_hum_txt), "%.2d", dataHum.ActualHumidity);
            snprintf(buffer_luz1_txt, sizeof(buffer_luz1_txt), "%.1d", led);
            snprintf(buffer_rssi_txt, sizeof(buffer_rssi_txt), "%d", rssi);
            ESP_LOGI("Muestra: ", "T %s - RSSI %s", buffer_temp_txt, buffer_rssi_txt);

            bufferJson[0] = 0;
            strcat(bufferJson, "{\"dispositivoId\":");
            strcat(bufferJson, IOTCORE_DEVICE_NUMBER);
            strcat(bufferJson, ",");
            strcat(bufferJson, "\"nombre\":\"ESP32_TEMP\"");
            strcat(bufferJson, ",");
            strcat(bufferJson, "\"ubicacion\":\"Comedor\"");
            strcat(bufferJson, ",");
            strcat(bufferJson, "\"luz1\":");
            strcat(bufferJson, buffer_luz1_txt);
            strcat(bufferJson, ",");
            strcat(bufferJson, "\"luz2\":0");
            strcat(bufferJson, ",");
